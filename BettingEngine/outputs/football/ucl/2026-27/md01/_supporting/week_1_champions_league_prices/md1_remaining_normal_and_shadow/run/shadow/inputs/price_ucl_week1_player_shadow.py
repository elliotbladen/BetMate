...
def adjust_over25(p_over, lam, mu, shadow_lam, shadow_mu):
    """Shift the baseline O/U 2.5 probability by the shadow's Poisson odds ratio.

    Same odds-ratio construction as the EPL/EFL shadow, so the player residual moves
    totals only as far as the change in expected goals justifies and the baseline's
    own (uncalibrated) level is carried through rather than replaced.
    """
    base, shadow = poisson_markets(lam, mu), poisson_markets(shadow_lam, shadow_mu)
    for value in (base['over25'], shadow['over25'], p_over):
        if not 0 < value < 1:
            raise ValueError('Invalid O/U 2.5 input to the player-shadow adjustment')
    odds = p_over / (1 - p_over) * (shadow['over25'] / (1 - shadow['over25'])) / (base['over25'] / (1 - base['over25']))
    adjusted = odds / (1 + odds)
    if not np.isfinite(adjusted) or not 0 < adjusted < 1:
        raise ValueError('Invalid player-shadow O/U 2.5 probability')
...
def shadow_price(normal, features, model):
    ...
    result={'expected_goals':{'home':sh,'away':sa},'delta_goals':dict(zip(('home','away'),delta.tolist())),
            'probabilities':dict(zip(('home','draw','away'),prob.tolist())),
            'fair_odds':dict(zip(('home','draw','away'),(1/prob).tolist()))}
    totals=normal.get('totals_over_under_25')
    if totals:
        result['totals_over_under_25']={}
        for track,values in totals.items():
            over=adjust_over25(values['p_over'],lam,mu,sh,sa)
            result['totals_over_under_25'][track]={'p_over':over,'p_under':1-over,
                'fair_odds':{'over':1/over,'under':1/(1-over)}}
    return result
...
def main():
    ...
    for row in rows:
        ...
        for kind in ('normal','shadow'):
            ...
            totals=(row[kind] or {}).get('totals_over_under_25') or {}
            for track,values in totals.items():
                item[f'{kind}_over25_{track}']=values['p_over']
                item[f'{kind}_over25_fair_odds_{track}']=values['fair_odds']['over']
                item[f'{kind}_under25_fair_odds_{track}']=values['fair_odds']['under']
