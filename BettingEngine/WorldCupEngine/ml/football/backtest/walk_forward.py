"""
Walk-forward backtest engine — league-parameterised (see ml/football/leagues/*.yaml).
...
Usage:
    python ml/football/backtest/walk_forward.py
"""
...
from ml.football.models.dixon_coles import (
    fit as dc_fit,
    expected_goals,
...
def _configure(league_key: str) -> None:
    ...
    MIN_TRAIN   = int(cfg.model["min_train"])
...
def load_and_merge() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if GOALS_FED or XG_CSV is None:
        # Goals-fed league (e.g. Championship): D-C fits on actual goals.
        merged = results.copy()
        merged["home_xg"] = merged["FTHG"].astype(float)
        merged["away_xg"] = merged["FTAG"].astype(float)
        print(f"Loaded {len(merged)} matches — goals-fed (no xG source)")
    else:
        ...
        merged["home_xg"] = merged["home_xg"].fillna(merged["FTHG"] * XG_FALLBACK)
        merged["away_xg"] = merged["away_xg"].fillna(merged["FTAG"] * XG_FALLBACK)
    ...
    if PPDA_CSV is not None and PPDA_CSV.exists():
...
def precompute_snapshots(
    ...
    for i, cutoff in enumerate(cutoff_dates):
        ...
        # D-C fit on xG (or goals for goals-fed leagues)
        dc_input = data_up_to.rename(columns={dc_col_home: "home_team", dc_col_away: "away_team"})
        dc = dc_fit(dc_input, as_of=cutoff, prev_ratings=prev_dc, rho=RHO, decay_rate=DECAY_RATE)
        prev_dc = dc

        # Elo from history
        elo = build_from_history(data_up_to, as_of=cutoff, **ELO_KWARGS)
