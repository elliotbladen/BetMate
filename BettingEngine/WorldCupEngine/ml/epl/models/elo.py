...
class EloTracker:
    ...
    def _k(self, team: str) -> float:
        ...
        if gp < K_NEW_THRESHOLD:
            # Linearly decay from K_NEW to K_BASE as games accumulate
            frac = gp / K_NEW_THRESHOLD
            return K_NEW * (1 - frac) + K_BASE * frac
        return K_BASE
    ...
    def update(
        ...
        exp_h = self._expected(elo_h + HOME_ADVANTAGE, elo_a)
        exp_a = 1.0 - exp_h
    ...
    def between_season_reversion(self) -> None:
        ...
        self.ratings = {
            t: max(MIN_REVERSION_ELO,
                   v * (1 - BETWEEN_SEASON_REVERSION) + mean * BETWEEN_SEASON_REVERSION)
...
def build_from_history(df: pd.DataFrame, as_of: datetime | None = None) -> EloTracker:
    """
