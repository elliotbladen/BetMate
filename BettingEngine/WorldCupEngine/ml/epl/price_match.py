"""
EPL match pricer — production script.

Usage:
    python ml/epl/price_match.py --home "Arsenal" --away "Chelsea"

    # With market odds for CLV
    python ml/epl/price_match.py --home "Man City" --away "Liverpool" \\
        --mkt-home 2.20 --mkt-draw 3.60 --mkt-away 3.20 --mkt-over25 1.85

    # With referee
    python ml/epl/price_match.py --home "Arsenal" --away "Chelsea" --ref "M Oliver"
...
from ml.epl.models.dixon_coles import fit as dc_fit, expected_goals, build_scoreline_matrix, derive_markets
from ml.epl.models.elo import build_from_history
from ml.epl.models.tiers import (
...
RESULTS_CSV = DATA_DIR / "clv" / "backtest_results.csv"
...
def load_data():
    ...
    ppda = pd.DataFrame()
    if PPDA_CSV.exists():
        ppda = pd.read_csv(PPDA_CSV, parse_dates=["date"])
...
def price_match(
    ...
    ratings = dc_fit(dc_input, as_of=as_of)
    ...
    elo = build_from_history(df[df["Date"] < as_of], as_of=as_of)
    ...
    adj = apply_all_tiers(lam_base, mu_base, context)
    lam = adj.lam_final
    mu  = adj.mu_final
...
def main():
    parser = argparse.ArgumentParser(description="Price an EPL match")
