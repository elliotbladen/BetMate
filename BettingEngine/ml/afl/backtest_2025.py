...
# Margin + Total models include EMA/opposition-adjusted features (improve MAE)
FEATURE_COLS_MARGIN_TOTAL = [
    'season_year',
...
# H2H classifier excludes EMA features — they hurt classification accuracy
FEATURE_COLS_H2H = [
    'season_year',
    'elo_diff', 'elo_win_prob',
    'home_rest_days', 'away_rest_days', 'rest_diff',
...
def main():
    ...
    # Prepare features — separate feature sets for margin/total vs H2H
    def prep_X(df, cols):
        feat = df[cols].copy()
        if 'mkt_home_prob_open' in feat.columns and 'elo_win_prob' in feat.columns:
            feat['mkt_home_prob_open'] = feat['mkt_home_prob_open'].fillna(feat['elo_win_prob'])
        return feat.fillna(0)

    X_mt  = prep_X(test_df, FEATURE_COLS_MARGIN_TOTAL)
    X_h2h = prep_X(test_df, FEATURE_COLS_H2H)
    test_df = test_df.copy()
    test_df['ml_margin']    = margin_model.predict(X_mt)
    test_df['ml_total']     = total_model.predict(X_mt)
    test_df['ml_home_prob'] = h2h_model.predict_proba(X_h2h)[:, 1]
