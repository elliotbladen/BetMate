...
SPLIT (with default --min-year 2022):
    min_year <= season <= 2023  →  split = 'train'
    season == 2024              →  split = 'validate'
    season == 2025              →  split = 'test'
    season >= 2026              →  split = 'deploy'  (no targets)
    season == 2020              →  excluded entirely (COVID)
    season < min_year           →  ELO walk-forward only; not in output CSV

ELO is always walked forward from the earliest available data so that
2022+ ratings are accurate — only the output records are filtered.

USAGE
-----
    python3 ml/afl/game_log.py
    python3 ml/afl/game_log.py --xlsx /path/to/afl.xlsx
    python3 ml/afl/game_log.py --min-year 2022   (default — modern-rule era)
    python3 ml/afl/game_log.py --min-year 2009   (full history)
"""
...
# Form config
# ---------------------------------------------------------------------------
FORM_GAMES       = 8         # look-back window (extended for EMA)
EMA_DECAY        = 0.75      # per-game decay: newest=1.0x, each step back ×0.75
...
def update_elo(home_elo: float, away_elo: float,
    ...
    # Log-based with diminishing returns; normalised so 30pts = 1.0× (AFL median margin).
    # Research: 538, fitzRoy and academic literature all recommend log/power over linear
    # because the difference between 30→39pts is less informative than 21→30pts.
...
def build_features(xlsx_path: Path, min_year: int = 2022) -> list[dict]:
    """
    Parse xlsx, walk-forward, return list of feature dicts.

    ELO is accumulated across ALL historical games so ratings are accurate.
    Only games with season >= min_year (and season != 2020) are written to
    the output records. 2020 (COVID) is always excluded.
    """
    rows = load_rows(xlsx_path)
    print(f'Loaded {len(rows)} games from {xlsx_path.name}')
    print(f'ELO warm-up: full history. Output records: season >= {min_year}, excluding 2020.')
    ...
    for idx, r in enumerate(rows):
        ...
        def get_form(team: str) -> dict:
            ...
            if not games:
                return {'win_pct': 0.5, 'avg_margin': 0.0, 'last_margin': 0.0,
                        ...
                        'ema_win_pct': 0.5, 'ema_margin': 0.0, 'opp_adj_margin': 0.0}
            n         = len(games)
            last_g    = games[-1]

            # EMA weights: oldest → EMA_DECAY^(n-1), newest → 1.0
            ...
            # Opposition-adjusted margin: scale each game's margin by opponent quality
            # adj_margin = margin × (opponent_elo / ELO_BASE)
            # Stronger opponent → margin worth more; weaker opponent → discounted
            opp_adj_margin = sum(
                w * g['margin'] * (g.get('opponent_elo', ELO_BASE) / ELO_BASE)
                for w, g in zip(weights, games)
            ) / w_sum

            # Plain averages (kept for backward compatibility)
            ...
            for g in reversed(games):
                if g['win']:
                    if lstreak == 0: wstreak += 1
                    else: break
                else:
                    if wstreak == 0: lstreak += 1
                    else: break
            ...
            return {
                ...
                'win_streak':     wstreak,
                'loss_streak':    lstreak,
                'ema_win_pct':    ema_win_pct,
                'ema_margin':     ema_margin,
                'opp_adj_margin': opp_adj_margin,
            }
        ...
        rec = {
            ...
            # EMA form (exponential moving average — recency-weighted, 8-game window)
            'home_ema_win_pct':      round(home_form['ema_win_pct'], 3),
            'home_ema_margin':       round(home_form['ema_margin'], 2),
            'home_opp_adj_margin':   round(home_form['opp_adj_margin'], 2),
            'away_ema_win_pct':      round(away_form['ema_win_pct'], 3),
            'away_ema_margin':       round(away_form['ema_margin'], 2),
            'away_opp_adj_margin':   round(away_form['opp_adj_margin'], 2),
            'ema_margin_diff':       round(home_form['ema_margin'] - away_form['ema_margin'], 2),
            'opp_adj_margin_diff':   round(home_form['opp_adj_margin'] - away_form['opp_adj_margin'], 2),
        ...
        # Only include in output if season is in the target era (ELO still updates below)
        if season >= min_year and season != 2020:
            records.append(rec)
...
def main():
    ...
    parser.add_argument('--min-year', default=2022, type=int,
                        help='Earliest season included in output (default 2022 = modern-rule era). '
                             'ELO walks forward from all available history regardless.')
    ...
    records = build_features(xlsx_path, min_year=args.min_year)
