...
> **Refactored 2026-07-09 (league-parameterised):** `ml/epl/` became `ml/football/` —
> one engine, N league configs in `leagues/*.yaml`. All entry points take `--league`
> (default `epl`). Regression gate passed: RPS 0.1319/0.1399/0.1287 reproduced exactly.

```
ml/football/
├── price_match.py              ← production entry point (--league epl)
├── ARCHITECTURE.md             ← this file
├── league_config.py            — YAML config loader (LeagueConfig)
├── leagues/
│   ├── epl.yaml                — all EPL constants (data paths, model, elo, tiers)
│   └── championship.yaml       — (coming — see CHAMPIONSHIP_PLAN.md)
├── fetch/
...
├── models/
│   ├── dixon_coles.py          — D-C model (xG-fed or goals-fed per league)
│   ├── elo.py                  — Elo model (league-parameterised constants)
│   ├── calibration.py          — isotonic calibration classes
│   └── tiers.py                — tier adjustment stack (T2–T7, TierParams per league)
├── backtest/
│   ├── walk_forward.py         — main backtest engine (--league epl)
...
```bash
# Minimal
python3 ml/football/price_match.py --home "Arsenal" --away "Chelsea"
...
```bash
python3 ml/football/fetch/fetch_results.py        # adds 2025/26 results
python3 ml/football/fetch/fetch_understat_xg.py   # adds 2025/26 xG
python3 ml/football/fetch/fetch_style_stats.py    # rebuilds PPDA
python3 ml/football/backtest/walk_forward.py      # regenerates calibration
```
