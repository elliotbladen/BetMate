...
Loads the trained XGBoost models, builds round features from the DB + historical
data, runs predictions, then layers the normal engine's T2-T8 adjustments on top.

Prints a side-by-side comparison:
    Rules model (T1–T8) vs ML baseline vs ML + T2–T8 overlays
...
def ensure_shadow_schema(conn: sqlite3.Connection) -> None:
    """Keep older local DBs compatible with expanded all-tier shadow output."""
    existing = {
        row['name']
        for row in conn.execute("PRAGMA table_info(ml_shadow_predictions)").fetchall()
    }
    columns = {
        't3_hcap': 'REAL',
        't3_tot': 'REAL',
    ...
    for name, decl in columns.items():
        if name not in existing:
            conn.execute(f"ALTER TABLE ml_shadow_predictions ADD COLUMN {name} {decl}")
    conn.commit()
...
def main():
    ...
    matches = conn.execute("""
        SELECT m.match_id, m.home_team_id, m.away_team_id,
               ...
               tp.totals_T1, tp.totals_T2, tp.totals_T3, tp.totals_T4,
               tp.totals_T5          AS t5_tot,
               tp.totals_T6, tp.totals_T7,
               ...
               tp.t7_condition_type, tp.t7_dew_risk,
               wra.referee_id,
               ref.referee_name,
    ...
    for m in matches:
        ...
        t3_hcap   = float(m['t3_home_delta'] or 0.0) - float(m['t3_away_delta'] or 0.0)
        t3_tot    = float(m['totals_T3'] or 0.0)
        t4_hcap   = float(m['t4_handicap_delta'] or 0.0)
        t4_tot    = float(m['totals_T4'] or 0.0)
        t5_hcap   = float(m['t5_hcap']  or 0.0)
        t5_tot    = float(m['t5_tot']   or 0.0)
        t6_hcap   = float(m['t6_handicap_delta'] or 0.0)
        t6_tot    = float(m['totals_T6'] or 0.0)
        t7_hcap   = get_t7_hcap(m['match_id'], m['home_team_id'], m['away_team_id'])
        t7_tot    = float(m['totals_T7'] or 0.0)
        known_rules_totals = (
            float(m['totals_T1'] or 0.0) + t2_tot + t3_tot + t4_tot
            + t5_tot + t6_tot + t7_tot
        )
    ...
    for r in results:
        ...
        p(f"  {game:<44} {r['_elo_diff']:>+7.1f} {r['ml_margin']:>+8.1f} "
          f"{r['t2_hcap']:>+5.1f} {r['t3_hcap']:>+5.1f} {r['t4_hcap']:>+5.1f} "
          f"{r['t5_hcap']:>+5.1f} {r['t6_hcap']:>+5.1f} {r['t7_hcap']:>+5.1f} "
          f"{r['ml_adj_margin']:>+10.1f} {r['rules_margin']:>+8.1f} "
          f"{diff:>+6.1f}{arrow}")
    ...
    for r in results:
        ...
        p(f"  {game:<44} {r['ml_total']:>8.1f} "
          f"{r['t2_tot']:>+5.1f} {r['t3_tot']:>+5.1f} {r['t4_tot']:>+5.1f} "
          f"{r['t5_tot']:>+5.1f} {r['t6_tot']:>+5.1f} {r['t7_tot']:>+5.1f} {r['t8_tot']:>+5.1f} "
    ...
    for r in results:
        ...
        rows_db.append({
            ...
            'elo_diff': r['_elo_diff'],
            'ml_raw_margin': r['ml_margin'],
            'ml_raw_total': r['ml_total'],
            'ml_raw_h2h_prob': r['ml_h2h_prob'],
            't2_hcap': r['t2_hcap'],
            't2_tot': r['t2_tot'],
            't3_hcap': r['t3_hcap'],
            't3_tot': r['t3_tot'],
            ...
            't7_tot': r['t7_tot'],
            't8_tot': r['t8_tot'],
            'ml_adj_margin': r['ml_adj_margin'],
            'ml_adj_total': r['ml_adj_total'],
            'ml_adj_h2h_prob': r['ml_adj_h2h_prob'],
            'rules_margin': r['rules_margin'],
            'rules_total': r['rules_total'],
            'rules_h2h_prob': r['rules_h2h_prob'],
            'margin_diff': m_diff,
            'total_diff': t_diff,
            'h2h_diff': h_diff,
            'agreement_flag': flag,
            ...
            'ml_margin_error': None,
            'ml_total_error': None,
            'ml_h2h_correct': None,
            'rules_margin_error': None,
            'rules_total_error': None,
            'rules_h2h_correct': None,
            'created_at': run_date,
        })
    ...
    columns = [
        ...
        't2_hcap', 't2_tot', 't3_hcap', 't3_tot', 't4_hcap', 't4_tot',
        't5_hcap', 't5_tot', 't6_hcap', 't6_tot', 't7_hcap', 't7_tot', 't8_tot',
