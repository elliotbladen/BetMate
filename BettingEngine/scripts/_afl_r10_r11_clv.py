"""CLV for AFL R10 + R11 bets. Same method: CLV% = odds_taken / close_odds - 1."""
import sys, csv
...
# R10 = May 14-17, R11 = May 21-24
R10_DAYS = {14,15,16,17}
R11_DAYS = {21,22,23,24}
...
r10 = {k: v for k, v in afl_rows.items()
       if hasattr(k[0],"month") and k[0].month==5 and k[0].day in R10_DAYS}
r11 = {k: v for k, v in afl_rows.items()
       if hasattr(k[0],"month") and k[0].month==5 and k[0].day in R11_DAYS}
print(f"  R10 games: {len(r10)}  |  R11 games: {len(r11)}")
...
with open(BETS, encoding="utf-8-sig") as f:
    bets = [b for b in csv.DictReader(f)
            if b["sport"]=="AFL" and b["round"] in ("10","11")]
print(f"  Bets: {len(bets)} ({sum(1 for b in bets if b['round']=='10')} R10, {sum(1 for b in bets if b['round']=='11')} R11)\n")

# ── load existing master to avoid duplicates ───────────────────────────────────
...
for b in bets:
    ...
    placed = b.get("placed_date","")

    xlsx_rows = r10 if rnd == "10" else r11
    g = find(xlsx_rows, home, away)
    hs, as_ = short(home), short(away)

    print(f"\n{bid}  AFL R{rnd} | {hs} vs {as_} | {mkt.upper()} | {sel}  odds:{odds}  line:{line}  [{result}]")

    if g is None:
        print(f"  ⚠ Not found in xlsx")
        new_master_rows.append({
            "bet_id":bid,"sport":"AFL","round":rnd,
    ...
    act_t = round(h_sc+a_sc,1) if h_sc is not None and a_sc is not None else None
    ...
    print(f"  CLV: {pct(clv_p)}  {verdict}  |  actual: {hs} {h_sc}-{a_sc} {as_} (total {act_t})")
    ...
    new_master_rows.append({
        "bet_id":bid,"sport":"AFL","round":rnd,
        "match":f"{hs} v {as_}","market":mkt,"selection":sel,
        "line":line,"odds_taken":odds,
        "open_line":open_l,"close_line":close_l,"line_move":lm,
        "close_odds":close_o,"clv_pct":clv_p,"clv_line":"",
...
# ── append new rows to master (skip dupes) ─────────────────────────────────────
added = 0
for r in new_master_rows:
    if r["bet_id"] not in master_ids:
        master_rows.append(r)
        master_ids.add(r["bet_id"])
        added += 1

with open(MASTER, "w", newline="", encoding="utf-8") as f:
    w = csv.DictWriter(f, fieldnames=master_fields, extrasaction="ignore")
    w.writeheader(); w.writerows(master_rows)
print(f"\nMaster: {added} new rows added  ({len(master_rows)} total)")
print("Now run update_clv_running.py to rebuild totals.")
