"""
AFL Halftime H2H Matrix Builder
Generates one sheet per AFL team showing actual win % vs halftime market implied win %.
Source: data/inplay/afl/halftime/processed/halftime_dataset.csv (seasons 2022–2026)
Output: outputs/afl_ht_h2h_matrix.xlsx

Same structure as nrl_ht_h2h_matrix.py — overall, halftime score position,
day of week, form, rest, moon phase, by month, H2H vs opponent.
"""
...
SOURCE_PATH = str(_ROOT / "data" / "inplay" / "afl" / "halftime" / "processed" / "halftime_dataset.csv")
OUTPUT_PATH = str(_ROOT / "outputs" / "afl_ht_h2h_matrix.xlsx")
...
# Drop non-AFL rows (pre-season state games etc.)
_DROP_TEAMS = {"Western Australia", "Victoria", "Vic Metro", "Vic Country"}
...
def load_data():
    ...
    # Parse dates (mixed format: DD/MM/YYYY or ISO with timezone)
    def _norm_date(s):
        s = str(s)
        if "/" in s:
    ...
    df["game_date"] = df["date"].apply(_norm_date)
    ...
    for _, r in df.iterrows():
        rows.append({
            ...
            "ht_leader":     str(r["ht_leader"]) if pd.notna(r.get("ht_leader")) else "level",
        })
...
def build_team_sheet(wb, team, all_rows, all_teams):
    ...
    tc = ws.cell(row=1, column=1, value=f"{team} — AFL Halftime H2H Matrix (2022–2026)")
    ...
    # ── DAY OF WEEK ──────────────────────────────────────────────
    section("DAY OF WEEK")
    row("Thursday Games",  [x for x in g if x["is_thursday"]])
    row("Friday Games",    [x for x in g if x["is_friday"]], alt=True)
    row("Saturday Games",  [x for x in g if x["is_saturday"]])
    row("Sunday Games",    [x for x in g if x["is_sunday"]], alt=True)
...
def main():
    print("Loading AFL halftime dataset...")
    all_rows = load_data()
