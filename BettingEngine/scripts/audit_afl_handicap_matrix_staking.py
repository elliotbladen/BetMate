#!/usr/bin/env python3
"""Audit actual 2026 AFL handicap bets against the frozen handicap matrix."""
...
MATRIX = ROOT / "outputs" / "afl_handicap_matrix.xlsx"
OUT_DIR = ROOT / "outputs" / "results"
THRESHOLDS = (10, 20, 30)


def selected_team(bet: base.Bet, game: dict) -> str | None:
    """Pick the backed side only if it is explicit in the market text."""
    teams = [team for team in afl.teams_in(bet.market) if team in (game["home"], game["away"])]
    return teams[0] if len(teams) == 1 else None


def handicap_line(market: str) -> float | None:
    # A leading +/− number is the only unambiguous handicap notation.  "Line
    # 10.5 Sydney" lacks the sign, so it is deliberately excluded.
    match = re.search(r"(?<![\w.])([+-]\d+(?:\.\d+)?)", market)
    return float(match.group(1)) if match else None


def applicable_rows(game: dict, team: str, games: list[dict], line: float) -> list[str]:
    ...
    if rest is not None:
        ...
        else: out.append("Normal Rest (7-9 days)")
    ...
    out.append("As Favourite (line < 0)" if line < 0 else "As Underdog  (line > 0)")
    if line <= -20: out.append("Heavy Fav (line ≤ -20)")
    elif line < 0: out.append("Slight Fav (line -0.5 to -19.5)")
    elif line < 20: out.append("Slight Dog (line +0.5 to +19.5)")
    else: out.append("Big Dog    (line ≥ +20)")
...
def main() -> None:
    ...
    for bet in afl.parse_bets():
        line = handicap_line(bet.market)
        if line is None:
            continue  # H2H and totals are outside this audit.
        game = afl.fixture_for(bet, games)
        if not game:
            exclusions.append((bet, "fixture not uniquely matched")); continue
        team = selected_team(bet, game)
        if not team:
            exclusions.append((bet, "backed side not explicit or unsigned line")); continue
        support, conflict = [], []
        for row in applicable_rows(game, team, games, line):
            value = matrix.get(afl.MATRIX_KEY[team], {}).get(row)
            if not value: continue
            edge, direction = value
            (support if direction == "covers" else conflict).append((edge, row, team))
        records.append({"bet": bet, "team": team, "line": line, "support": support, "conflict": conflict})

    ladders = {t: [r for r in records if qualifies(r, t)] for t in THRESHOLDS}
    actual = sum(base.pnl(r["bet"]) for r in records)
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    csv_path = OUT_DIR / "afl_2026_actual_handicap_matrix_staking_audit.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["id","date","match","market","team","line","odds","stake","result","support_10","support_20","support_30","conflict_10","conflict_20","conflict_30","support_rows","conflict_rows"])
        for r in records:
            b, s, c = r["bet"], r["support"], r["conflict"]
            w.writerow([b.ident,b.date,b.match,b.market,r["team"],r["line"],b.odds,b.stake,b.result,
    ...
    md_path = OUT_DIR / "afl_2026_actual_handicap_matrix_staking_audit.md"
    with md_path.open("w", encoding="utf-8") as fh:
        fh.write("# 2026 AFL actual handicap bets — matrix staking audit\n\n")
        fh.write("Source: BetMate's actual-bet ledger. Matrix: frozen 2022–25 AFL handicap matrix. A qualifier requires **3+ aligned signals** at the stated edge threshold and **no opposing signal** at that same threshold. The rows are overlapping, so this is confluence—not independent evidence.\n\n")
        ...
        fh.write(f"\n- Eligible, unambiguous 2026 handicap bets: **{len(records)}**; excluded: **{len(exclusions)}**.\n- Actual P&L across eligible bets: **${actual:+.2f}**.\n\n")
        for t, rs in ladders.items():
            ...
            fh.write("\n")
        fh.write("## Interpretation\n\nThis is an in-sample retrospective screen of a small, selected set of bets. It cannot justify a live stake increase. Freeze one rule and prospectively log every qualifying and non-qualifying handicap next season before changing Kelly/staking.\n")
