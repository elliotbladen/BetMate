"""
AFL Totals Matrix -- Melbourne Demons vs GWS Giants
R12 2026 -- Sunday 31 May, MCG, ~15:20 AEST

Rules model: 190.9 (+14.4 above market)
ML model:    ~178 (near market)
Market total: 176.5

Melbourne: 7 days rest, prev LOSS (May 24 lost to Bulldogs 90-93, away)
GWS Giants: 7 days rest, prev WIN (May 24 beat Brisbane 166-88, home, +78pt margin)
GWS injury load: Tom Green (season), Sam Taylor, Josh Kelly, Xavier O'Halloran,
                 Logan Smith, Cody Angove -- six outs (T5 +3.5 net to Melbourne)
Melbourne outs: Jack Viney (elite mid), Christian Salem (good KD)

May 31 = full moon (nearest full May 31, is_full_moon = True)
"""
...
GAME_TIME      = datetime(2026, 5, 31, 15, 20)   # Sunday afternoon MCG
HOME_TEAM      = "Melbourne"
AWAY_TEAM      = "GWS Giants"
VENUE          = "MCG"
MARKET_TOTAL   = 176.5

MELBOURNE_PREV_WIN = False   # lost to Western Bulldogs 90-93 (away, May 24)
GWS_PREV_WIN       = True    # beat Brisbane 166-88 (home, May 24, +78pt margin)
MELBOURNE_REST     = 7       # May 24 -> May 31
GWS_REST           = 7       # May 24 -> May 31
...
def main():
    print("=" * 120)
    print(f"  AFL Totals Matrix -- {HOME_TEAM} Demons vs {AWAY_TEAM}")
    print(f"  {GAME_DATE.strftime('%A %d %B %Y')} | {VENUE} | Market total: {MARKET_TOTAL}")
    print(f"  Rules: 190.9 (+14.4 above market) | ML: ~178 (near market) | Threshold: {EDGE_THRESHOLD:.0f}%+")
    print("=" * 120)
    ...
    print(f"  DAY:  {GAME_TIME.strftime('%A')} afternoon (thu_fri={wd in (3,4)}, sat={wd==5}, sun={wd==6}, night={GAME_TIME.hour>=17})")
    print(f"  FORM: Melbourne prev LOSS ({MELBOURNE_REST}d rest) | GWS Giants prev WIN ({GWS_REST}d rest, +78pt margin)")
    print(f"  NOTE: GWS 6 outs (Green/Taylor/Kelly/O'Halloran/Smith/Angove). Melbourne: Viney + Salem out.")
    ...
    for team in [HOME_TEAM, AWAY_TEAM]:
        ...
        prev_win  = MELBOURNE_PREV_WIN if team == HOME_TEAM else GWS_PREV_WIN
        rest_days = MELBOURNE_REST if team == HOME_TEAM else GWS_REST
