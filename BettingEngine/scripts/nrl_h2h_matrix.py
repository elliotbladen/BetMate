...
Source: BetMate/data/nrl/historical/latest.xlsx (seasons 2022–2025)
...
_BETMATE = Path(os.environ.get("BETMATE_ROOT", "")) if os.environ.get("BETMATE_ROOT") else _ROOT.parent / "BetMate"

SOURCE_PATH = str(os.environ.get("NRL_HISTORICAL_XLSX") or _BETMATE / "data/nrl/historical/latest.xlsx")
OUTPUT_PATH = str(_ROOT / "outputs" / "nrl_h2h_matrix.xlsx")
...
_NRL_TEAM_STATE = {
    "Brisbane Broncos":       "QLD", "Gold Coast Titans":      "QLD",
    "North QLD Cowboys":      "QLD", "Dolphins":               "QLD",
    "Canberra Raiders":       "NSW", "Canterbury Bulldogs":    "NSW",
    "Cronulla Sharks":        "NSW", "Manly Sea Eagles":       "NSW",
    "Newcastle Knights":      "NSW", "Parramatta Eels":        "NSW",
    "Penrith Panthers":       "NSW", "South Sydney Rabbitohs": "NSW",
    "St George Dragons":      "NSW", "Sydney Roosters":        "NSW",
    "Wests Tigers":           "NSW", "Melbourne Storm":        "VIC",
...
_NRL_VENUE_STATE = {
    ...
    # Long haul (Perth, Darwin, Las Vegas)
    "Allegiant Stadium": "LH", "HBF Park": "LH", "Optus Stadium": "LH", "TIO Stadium": "LH",
}


def _nrl_trip_type(game, team):
    """Classify an away game as 'local', 'interstate', or 'long_haul'."""
    vs = _NRL_VENUE_STATE.get(game.get("venue", ""), "LH")
...
def build_team_sheet(wb, team, all_rows, all_teams):
    ...
    row("Away — Local / Short Trip (same state)",         [x for x in away_g if _nrl_trip_type(x, team) == "local"], alt=True)
    row("Away — Interstate",                              [x for x in away_g if _nrl_trip_type(x, team) == "interstate"])
    row("Away — Long Haul (NZ / Perth / Darwin / Vegas)", [x for x in away_g if _nrl_trip_type(x, team) == "long_haul"], alt=True)
...
def main():
    ...
    import argparse
    ap = argparse.ArgumentParser(description="NRL h2h matrix builder")
